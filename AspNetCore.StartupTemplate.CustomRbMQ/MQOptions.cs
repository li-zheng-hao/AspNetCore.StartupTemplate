    ...
    public MQOptions()
        {
            ...
            Extensions = new List<ICustomRbMQOptionsExtension>();
            Version = "v1";
            DefaultGroupName = "customrbmq.queue." + Assembly.GetEntryAssembly()?.GetName().Name.ToLower();
            CollectorCleaningInterval = 300;
            UseDispatchingPerGroup = false;
        }

        internal IList<ICustomRbMQOptionsExtension> Extensions { get; }

        /// <summary>
        /// 订阅者默认的队列名 rabbitmq --> queue name.
        /// </summary>
        ...
        public void RegisterExtension(ICustomRbMQOptionsExtension extension)
        {
            if (extension == null)
            {
